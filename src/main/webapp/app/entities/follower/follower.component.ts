...
export class FollowerComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.followerService
      ...
      .subscribe(
        (res: HttpResponse<IFollower[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IFollower): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInFollowers(): void {
    this.eventSubscriber = this.eventManager.subscribe('followerListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IFollower[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
