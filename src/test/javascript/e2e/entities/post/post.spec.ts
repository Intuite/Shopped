...
describe('Post e2e test', () => {
  ...
  it('should load Posts', async () => {
    ...
    expect(await postComponentsPage.getTitle()).to.eq('shoppedApp.post.home.title');
  ...
  it('should load create Post page', async () => {
    ...
    expect(await postUpdatePage.getPageTitle()).to.eq('shoppedApp.post.home.createOrEditLabel');
  ...
  /* it('should create and save Posts', async () => {
        ...
        await promise.all([
            postUpdatePage.setCaptionInput('caption'),
            postUpdatePage.setDateInput('01/01/2001' + protractor.Key.TAB + '02:30AM'),
            postUpdatePage.statusSelectLastOption(),
            postUpdatePage.recipeSelectLastOption(),
            postUpdatePage.userSelectLastOption(),
        ]);

        expect(await postUpdatePage.getCaptionInput()).to.eq('caption', 'Expected Caption value to be equals to caption');
  ...
  /* it('should delete last Post', async () => {
        ...
        expect(await postDeleteDialog.getDialogTitle())
            .to.eq('shoppedApp.post.delete.question');
