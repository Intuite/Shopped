...
insert into catalogue (id, id_catalogue, value)
values (1, 'award_tax', 5);

insert into catalogue (id, id_catalogue, value)
values (2, 'cookie_withdraw_price', 40);
...
select @uauser := ua.user_id
from jhi_user_authority ua
         inner join jhi_user u on ua.user_id = u.id
where u.login = 'admin'
  and ua.authority_name like 'ROLE_USER';
delete
from jhi_user_authority
where user_id = @uauser
  and authority_name like 'ROLE_USER';
